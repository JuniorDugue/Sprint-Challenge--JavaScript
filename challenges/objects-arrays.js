...
console.log(`${velociraptor.name} has a ${velociraptor.diet} diet.`);

// How long was a stegosaurus?
console.log(`${stegosaurus.name} was ${stegosaurus.length}m long.`);

// What time period did tyrannosaurus live in?
console.log(`${tyrannosaurus.name} lived during the ${tyrannosaurus.period}.`);


// Create a new roar method for the tyrannosaurus.  When called, return "RAWERSRARARWERSARARARRRR!" Log the result.
console.log(`${tyrannosaurus.name} roars, ${tyrannosaurus.roar()}`);
...
for (let i = 0; i < graduates.length; i++) {
  let regEx = /Uni/;
  if (graduates[i].university.match(regEx)) uni.push(graduates[i].university);
}
console.log(uni);
...
const populationTotal = zooAnimals.reduce(
  (accum, cur) => accum + cur.population,
  0
);
console.log(populationTotal);
